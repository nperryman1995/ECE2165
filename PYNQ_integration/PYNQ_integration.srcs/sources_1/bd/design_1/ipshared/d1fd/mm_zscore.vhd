...
architecture structure of mm_zscore is
	-- user logic
    
	-- template
	constant NUM_REGS   : positive := (2 ** MAX_DATA_PTS_EXP) + 7;
...
begin
	...
	-- start of user logic
	alsdlsad: for i in 7 to NUM_REGS-1 generate
		DataOut((DATA_WIDTH * (i-6)) - 1 downto (DATA_WIDTH * (i-7))) <= slv_out(i);
		slv_in(i) <= slv_out(i);
	end generate;
	CountOut <= slv_out(1);
	slv_in(1) <= slv_out(1);
	...
	injErr <= slv_out(3);
	slv_in(3) <= slv_out(3);
    errCtrlAdd1 <= slv_out(4)((3*8)-1 downto 0);
    slv_in(4) <= slv_out(4);
    errCtrlAdd2 <= slv_out(5)((3*8)-1 downto 0);
    slv_in(5) <= slv_out(5);
    errCtrlSub <= slv_out(6)((3*8)-1 downto 0);
    slv_in(6) <= slv_out(6);
	-- registered output
