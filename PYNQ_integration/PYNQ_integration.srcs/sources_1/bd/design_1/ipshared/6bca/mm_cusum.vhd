...
architecture structure of mm_cusum is
	...
	-- template
	constant NUM_REGS   : positive := (2 ** MAX_DATA_PTS_EXP) + (2 ** MAX_MEANS_EXP) + 4;
...
begin
	...
	-- start of user logic
	alsdlsad: for i in 4 to NUM_REGS-1 - (2 ** MAX_MEANS_EXP) generate
		DataOut((DATA_WIDTH * (i-3)) - 1 downto (DATA_WIDTH * (i-4))) <= slv_out(i);
		slv_in(i) <= slv_out(i);
	end generate;
    alsdlsadad: for i in 4 + (2 ** MAX_DATA_PTS_EXP) to NUM_REGS-1 generate
		MeansOut((DATA_WIDTH * (i-(3 + (2 ** MAX_DATA_PTS_EXP)))) - 1 downto (DATA_WIDTH * (i-(4 + (2 ** MAX_DATA_PTS_EXP))))) <= slv_out(i);
		slv_in(i) <= slv_out(i);
	end generate;
	CountOut <= slv_out(1);
	slv_in(1) <= slv_out(1);
